    ...
    public void getHDFSQuota() {
        ...
        try {
            ...
            for (String hdfsPath : FOLDERS_TO_MONITOR_HDFS_QUOTA.split(",")) {
                System.out.println("hdfsPath is "+hdfsPath);
                ...
                for (FileStatus fileStat : fileStatus) {
                    if (fileStat.isDirectory()) {
                        ...
                        int id = 0;
                        if (hdfsQuotaInfo2!=null)
                            id= (Integer) session.save(hdfsQuotaInfo2);
                        System.out.println("inserted record in hdfs Quota database with id is "+id);
